<?php
	...
	public function logarUsuario($usuario){
		...
		INNER JOIN consumidores AS CON ON (CON.USU_ID = USU.USU_ID)
		WHERE usu.USU_EMAIL = :USU_EMAIL AND 
		usu.USU_SENHA = :USU_SENHA AND
		con.CON_CADASTRO_VALIDADO = 1;";	
		...
		*/	
		...
		public function alterarSenha($retorno){
		$this->sql="UPDATE USU_SENHA SET $retorno FROM usuarios WHERE $idUsuarioBusca = :USU_ID";
		$this->resultado = $this->conexao->prepare($this->sql);
