...
class UsuarioDAO 
{
	...
	public function inserirUsuario($usuario)
	{
		...
		$this->sql="insert into $this->tabela (USU_FOTO_PERFIL, USU_DATA_CADASTRO, USU_EMAIL, USU_TELEFONE, USU_SENHA, USU_NOME, USU_CADASTRO_VALIDADO, USU_TIPO) values (:USU_FOTO_PERFIL, :USU_DATA_CADASTRO, :USU_EMAIL, :USU_TELEFONE, :USU_SENHA, :USU_NOME, 0, 1)";
		$this->resultado = $this->conexao->prepare($this->sql);


		$this->resultado->bindValue(':USU_FOTO_PERFIL',$this->usuario->getUSU_FOTO_PERFIL());     
	...
	{
		$this->sql="select * from usuarios order by USU_ID desc limit 4";
		$this->resultado= $this->conexao->prepare($this->sql);
		$this->resultado->execute();
		return $this->resultado->fetchAll();	 
	}
	
	public function usuariosPorMes($mes)
	{
		$this->sql="select * from usuarios where MONTH(USU_DATA_CADASTRO) = ".$mes;
		$this->resultado= $this->conexao->prepare($this->sql);
		$this->resultado->execute();
		return $this->resultado->fetchAll();
	}
	...
	public function logarUsuario($usuario)
	{
		...
		if ($this->resultado->rowCount() > 0) 
		{
			...
			$ID = $usrLogado->getUSU_ID();
			
			$EST_ID = $this->idEst($ID);
			$this->sql="SELECT * FROM historicos_bloqueio_estabelecimento WHERE EST_ID = :EST_ID AND NOW() < HBE_DATA_HORA_DESBLOQUEIO";	
			$this->resultado = $this->conexao->prepare($this->sql);
			...
			if (($this->resultado->rowCount()) > 0) {
				...
				session_start();
				$_SESSION['loginErro2'] = "<br><br>Você foi bloqueado de nosso sistema!";
				header('location:../../usu_loginUsuario.php');
				exit();
			}
			
			$this->sql="SELECT * FROM historico_acesso_usuario WHERE USU_ID = :USU_ID";	
			$this->resultado = $this->conexao->prepare($this->sql);
			$this->resultado->bindParam(':USU_ID', $ID);
			$this->resultado->execute();
			if (($this->resultado->rowCount()) > 0) {
				
				$this->sql = "update historico_acesso_usuario set HIS_PLATAFORMA=:HIS_PLATAFORMA,HIS_DATA_ULTIMO_ACESSO=:HIS_DATA_ULTIMO_ACESSO where USU_ID=:USU_ID";
				$this->resultado = $this->conexao->prepare($this->sql);
				$this->resultado->bindParam(':HIS_PLATAFORMA', $_SERVER['HTTP_USER_AGENT']);
				date_default_timezone_set('America/Sao_Paulo');		
				$dataAtual = time();
				$this->resultado->bindParam(':HIS_DATA_ULTIMO_ACESSO', date("Y-m-d H:i:s",$dataAtual));
				$this->resultado->bindParam(':USU_ID', $ID);
				$this->resultado->execute();

			}
			else {
				$this->sql="INSERT INTO historico_acesso_usuario (HIS_PLATAFORMA, HIS_DATA_ULTIMO_ACESSO,USU_ID) values (:HIS_PLATAFORMA,:HIS_DATA_ULTIMO_ACESSO,$ID)";	
				$this->resultado = $this->conexao->prepare($this->sql);

				$this->resultado->bindValue(':HIS_PLATAFORMA', $_SERVER['HTTP_USER_AGENT']);     
				date_default_timezone_set('America/Sao_Paulo');		
				$dataAtual = time();
				$this->resultado->bindValue(':HIS_DATA_ULTIMO_ACESSO',date("Y-m-d H:i:s",$dataAtual));   
	...
	public function logarEstabelecimento($usuario)
	{
		...
		if ($this->resultado->rowCount() > 0) 
		{
			$this->sql="SELECT * FROM historicos_bloqueio_estabelecimento WHERE EST_ID = :EST_ID";	
			$this->resultado = $this->conexao->prepare($this->sql);
			$this->resultado->bindParam(':EST_ID', $EST_ID);
			$this->resultado->execute();
			if (($this->resultado->rowCount()) > 0)
	...
	public function buscarTipoId($email)
	{
		$idTipoUsuarioBusca = NULL;	

		$this->sql="SELECT * FROM usuarios WHERE USU_EMAIL = :USU_EMAIL";
		$this->resultado = $this->conexao->prepare($this->sql);
		...
		if ($this->resultado->rowCount() > 0) 
		{
			foreach($resultado as $linha) 
			{
				$idTipoUsuarioBusca = $linha['USU_TIPO'];
			}	
		}
		return $idTipoUsuarioBusca;
	}
		...
		$this->resultado->execute();
			...
			*/
				...
				public function salvar_senha($post) 
				{
					...
					$this->resultado->bindValue(':USU_SENHA',$this->post->getUSU_SENHA_NOVA());
				...
				{
					$this->post = $post;
					$this->sql = "update $this->tabela set USU_NOME =:USU_NOME, USU_TELEFONE =:USU_TELEFONE where USU_ID =:USU_ID";

					$this->resultado = $this->conexao->prepare($this->sql);
					
					$this->resultado->bindValue(':USU_NOME', $this->post->getUSU_NOME());
					$this->resultado->bindValue(':USU_TELEFONE', $this->post->getUSU_TELEFONE());
					$this->resultado->bindValue(':USU_ID', $this->post->getUSU_ID());
				...
				public function editarFotoPerfil($post) 
				{
					$this->post = $post;
					$this->sql = "update $this->tabela set USU_FOTO_PERFIL =:USU_FOTO_PERFIL where USU_ID =:USU_ID";

					$this->resultado = $this->conexao->prepare($this->sql);
					
					$this->resultado->bindValue(':USU_FOTO_PERFIL', $this->post->getUSU_FOTO_PERFIL());
					$this->resultado->bindValue(':USU_ID', $this->post->getUSU_ID());
				...
				public function buscarConsumidor($id)
				{
					$this->sql="select * from consumidores where USU_ID=$id";
					$this->resultado= $this->conexao->prepare($this->sql);
					$this->resultado->execute();
					return count($this->resultado->fetchAll());	
				}
				...
				public function selecionarNome($usu_id)
				{

					$this->sql = "select * from $this->tabela where USU_ID=$usu_id";
					$this->resultado = $this->conexao->prepare($this->sql);
					$this->resultado->execute();
					...
					return $linha_consumidor['USU_NOME'];
				}

				public function buscarIDEstabelecimento($id)
				{
					$this->sql="select EST_ID from estabelecimentos where USU_ID=$id";
					$this->resultado= $this->conexao->prepare($this->sql);
					$this->resultado->execute();
					$var=$this->resultado->fetchAll();
					foreach ($var as $key) {
						# code...
						return $key['EST_ID'];
					}
