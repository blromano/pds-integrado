...
class EstabelecimentosDAO 
{
	...
	public function pesquisarReclamacoesPorEstabelecimento($est_id) 
	{
		$this->sql = "select EST_TOTAL_RECLAMACAO from $this->tabela where EST_ID=:est_id";
		$this->resultado = $this->conexao->prepare($this->sql);
		$this->resultado->bindParam(':est_id',$est_id);
		$this->resultado->execute();
		foreach ($this->resultado->fetchAll() as $row){
			$total_reclamacao = $row["EST_TOTAL_RECLAMACAO"];
		}
		return $total_reclamacao;
	}
	...
	{
		$this->post = $post;
		$this->sql = "update $this->tabela set EST_NOME_FANTASIA=:EST_NOME_FANTASIA, EST_NOME_RESPONSAVEL=:EST_NOME_RESPONSAVEL, EST_FACEBOOK_EMPRESA=:EST_FACEBOOK_EMPRESA, EST_SITE_EMPRESA=:EST_SITE_EMPRESA, TES_ID=:TES_ID where EST_ID =:EST_ID";
	...
	{
		$this->sql = "SELECT * FROM ESTABELECIMENTOS LEFT JOIN USUARIOS ON ESTABELECIMENTOS.USU_ID = USUARIOS.USU_ID LEFT JOIN LOCALIZACOES ON ESTABELECIMENTOS.LOC_ID = LOCALIZACOES.LOC_ID LEFT JOIN TIPOS_ESTABELECIMENTOS ON ESTABELECIMENTOS.TES_ID = TIPOS_ESTABELECIMENTOS.TES_ID WHERE EST_STATUS_BLOQUEIO = 0";
		$this->resultado = $this->conexao->prepare($this->sql);
		$this->resultado->execute();
		return $this->resultado->fetchAll();
	}
	...
	public function obterEstabelecimentoPorId($id)
	{
		$this->sql = "select LOC_ID from $this->tabela WHERE EST_ID = :id";
		$this->resultado = $this->conexao->prepare($this->sql);
		$this->resultado->bindParam(':id',$id);
		$this->resultado->execute();
		$resultado = $this->resultado->fetchAll();
	...
	public function estabelecimentoNomeReclamacao($con_id) 
	{
		$this->sql = "SELECT EST_NOME_FANTASIA FROM estabelecimentos INNER JOIN reclamacoes ON estabelecimentos.EST_ID = reclamacoes.EST_ID WHERE CON_ID=:con_id";
		$this->resultado = $this->conexao->prepare($this->sql);
	...
	public function estabelecimentoNomeAvaliacao($con_id) 
	{
		$this->sql = "SELECT EST_NOME_FANTASIA FROM estabelecimentos INNER JOIN avaliacoes_estabelecimentos ON estabelecimentos.EST_ID = avaliacoes_estabelecimentos.EST_ID WHERE CON_ID=:con_id";
		$this->resultado = $this->conexao->prepare($this->sql);
		$this->resultado->bindParam(':con_id',$con_id);
		$this->resultado->execute();
		return $this->resultado->fetchAll();
	...
	public function contarEstabelecimentos()
	{
		$this->sql="select COUNT(EST_ID) as qtdEstabelecimentos from $this->tabela";
		$this->resultado= $this->conexao->prepare($this->sql);
		$this->resultado->execute();
		foreach ($this->resultado->fetchAll() as $linha)
		{
			return $linha['qtdEstabelecimentos'];	
		}
	}
	
	public function selecionarMediaAvaliacao($est_id)
	{
		$this->sql="select EST_MEDIA_AVALIACAO_CONSUMIDORES as media_avaliacao from $this->tabela WHERE EST_ID=:est_id";
		$this->resultado= $this->conexao->prepare($this->sql);
		$this->resultado->bindParam(':est_id',$est_id);
		$this->resultado->execute();
		foreach ($this->resultado->fetchAll() as $linha)
		{
			return $linha['media_avaliacao'];	
		}
	}
	
	public function selecionarNomeEstabelecimento($est_id)
	{
		$this->sql="select EST_NOME_FANTASIA from $this->tabela WHERE EST_ID=:est_id";
		$this->resultado= $this->conexao->prepare($this->sql);
		$this->resultado->bindParam(':est_id',$est_id);
		$this->resultado->execute();
		foreach ($this->resultado->fetchAll() as $linha)
		{
			return $linha['EST_NOME_FANTASIA'];	
		}
	}

	public function listarFeedbacks($id, $tipo) 
	{
		if ($tipo == "ESTABELECIMENTO") {
						# code...
			$this->sql = "select * from feedbacks_reunioes_agendadas where EST_ID = $id AND FEE_DATA_HORA IS NOT NULL;";
		}else if ($tipo == "CONSUMIDOR") {
			# code...
			$this->sql = "select * from feedbacks_reunioes_agendadas where CON_ID = $id AND FEE_DATA_HORA IS NOT NULL;";
		}else if ($tipo == "ADMINISTRADOR") {
			# code...
			$this->sql = "SELECT * FROM feedbacks_reunioes_agendadas where FEE_DATA_HORA IS NOT NULL";
		}else{
			return 0;
		}
		$this->resultado = $this->conexao->prepare($this->sql);
	...
	public function tendenciosos()
	{
		$this->sql = "SELECT * from estabelecimentos order by EST_TOTAL_RECLAMACAO DESC LIMIT 5;";
		$this->resultado = $this->conexao->prepare($this->sql);
		$this->resultado->execute();
