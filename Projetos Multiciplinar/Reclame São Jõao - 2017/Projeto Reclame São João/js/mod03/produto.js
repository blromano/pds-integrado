	...
	var teste; 
		...
		// Alterar Produto	
			$('#ALTERAR_PRODUTO').on('show.bs.modal', function (event) 
			{
				...
				modal.find('#recipient-tipo-produto').val(recipienttipo)
		...
		// Excluir Produto	
			$('#EXCLUIR_PRODUTO').on('show.bs.modal', function (event) 
			{
				...
				var modal = $(this)
				document.getElementById("recipient-nome-produto-exclusao").innerHTML = recipientvisualizarproduto;
				document.getElementById("id-numero-produto").innerHTML = recipientnum;
				
				modal.find('#recipient-numero-produto').val(recipientnum)
			});
			
		
		function teste(recipienttipo)
		{
			return recipienttipo;
		};