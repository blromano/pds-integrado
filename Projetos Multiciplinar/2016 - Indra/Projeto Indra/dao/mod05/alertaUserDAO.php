    ...
    try{
        ...
        for($i = $num; $i <= $qntImagens; $i++){
            if($i == $qntImagens)
                ...
                $query = "UPDATE alertas_enviados_usuarios SET " . $opcoesBanco[$i] . " = \"" . $caminhoPasta . "/" . $i. ".png\" WHERE ALU_ID = " . $id;
            $stmt = $db->prepare($query);
            if(!$stmt->execute())
                $result = false;
        }
...
public function listarByUser($idUser){
    try {
        $db = Database::conectar();
        $query = "SELECT * FROM alertas_enviados_usuarios WHERE USU_ID = :idUser";
        $stmt = $db->prepare($query);
