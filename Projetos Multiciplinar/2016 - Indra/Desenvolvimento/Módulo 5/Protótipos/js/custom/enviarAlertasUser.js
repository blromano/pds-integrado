$(document).ready(function() {
	...
	function listarAlertas() {
		$.ajax({
			url: "class/alertas/user/listarAlertasUser.php",
			dataType: "json",
	...
	function criarCarrosel(imgs){
		...
		caminho = caminho[0] + '/' + caminho[1] + '/' + caminho[2] + '/' + caminho[3] + '/';
	...
	$('#delImg').on('click', function() {
		...
		$.ajax({
			type: 'POST',
			url: 'class/alertas/user/deletarImagem.php',
			data: {
				caminho: caminhoImg,
				id: idImg
			},
	...
	$('#botaoEnviar').on('click', function() {	
		...
		if($('#formImg').data('formValidation').isValid()){
			...
			$.ajax({
				type: 'POST',
				url: 'class/alertas/user/addImageAlertaUser.php',
				data: data,
				contentType: false,
				processData: false,
				success: function(data) {
	...
	$('#simCadastrar').on('click', function() {
		...
		if($('#formCriar').data('formValidation').isValid()) {
			...
			$.ajax({
				type: 'POST',
				url: 'class/alertas/user/createAlertasUser.php',
				data: data,
				contentType: false,
				processData: false,
				success: function(data) {
		...
		$('#simEditar').click(function() {
			if(x){
				...
				if($('#formEditar').data('formValidation').isValid() || $('#formEditar').data('formValidation').isValid() == null) {
					$.ajax({
						method: 'POST',
						url: 'class/alertas/user/editarAlertasUser.php',
						data: alertaUser,
						success: function(data) {
	...
	$('#corpoTabela').on('click', 'button.btn-deletar', function() {
		...
		$('#deletar-sim').on('click', function() {
			if (x) {
				$.ajax({
					type: 'POST',
					url: 'class/alertas/user/deletarAlertaUser.php',
					data: info,
					success: function(data) {				
						if (data) {
