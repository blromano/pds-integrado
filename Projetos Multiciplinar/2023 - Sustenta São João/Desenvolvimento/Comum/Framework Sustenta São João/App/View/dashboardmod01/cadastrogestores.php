...
<div class="card">
    ...
    <div class="card-body">
        <form action="/inserirGestor" name="cadastro" class="form_j" method="POST" enctype="multipart/form-data" onsubmit="validaCampo()" id="my_form">
            <div class="form-row">
            ...
            <div class="form-row">
              <div class="form-group col-md-4">
                <label for="GES_PRONTUARIO">Prontuário</label><br>
                <input required type="tel" name="GES_PRONTUARIO" id="GES_PRONTUARIO" class="form-control text_j" placeholder="Digite seu Prontuário" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..?)\../g, '$1');">
              </div> 
            </div>
            <div class="form-row">
              <div class="form-group col-md-4">
                <label for="GES_RAMAL">Ramal</label><br>
                <input required type="tel" name="GES_RAMAL" id="GES_RAMAL" class="form-control text_j" placeholder="Digite seu Ramal" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..?)\../g, '$1');" maxlength="4">
              </div> 
            </div>
            <div class="row">
              <div class="form-group col-md-4">
                        <label for = "FK_SETORES_SET_ID">Setor</label>
                            <select name="FK_SETORES_SET_ID" class="form-control text_j" id="">
                                <option value="" selected disabled>Selecione um Setor</option>
                                <?php
                                  foreach($this->getView()->setores as $setor)
                                  {
                                    echo "<option value=";
                                    echo $setor->__GET("SET_ID");
                                    echo ">";
                                    echo $setor->__GET("SET_NOME");
                                    echo "</option>";
                                  }
                                ?>
                            </select>
...
<script>
    function validaCampo() {
        ...
        Swal.fire(
            'Enviado!',
            'O servidor foi cadastrado com sucesso.',
            'success'
        ).then(() => {
