<!DOCTYPE html> 
<html>
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title> +Saúde São João </title>
        <script src="<?= JQUERY_PATH ?>" charset="utf-8"></script>
        <script src="<?= POPPER_PATH ?>" charset="utf-8"></script>
        <script src="<?= THEME_JS_PATH ?>" charset="utf-8"></script>
        <script src="<?= ANIMATIONS_PATH ?>" charset="utf-8"></script>
        <link rel="stylesheet" type="text/css" href="hover.css">
        <link rel="stylesheet" href="<?= THEME_CSS_PATH ?>">
    </head>
    <body>
        <?php
        $vertical_menu_links = [
            "Nome do link" => "link_de_redirecionamento.html"
        ];
        include(VIEWS_PATH . '/module_base.php');
        ?>
        <section onclick="closeNav()">
            <div class="container" role="main" style="height: 100%;width: 100%; padding:5%">

                <div class="page-header">
                    <h1 style="color:#104999">Gastos calóricos
                    </h1>
                </div>
                <a href='?mod=rtreinos&sub=adicionar_gasto_calorico'><button>Adicionar gasto calórico</button> </a>
                <div class="container">
                    <table class="table">
                        <thead>
                            <tr>
                                <th>Data</th>
                                <th>Exercício realizado</th>
                                <th>Gastos calóricos</th>
                                <th></th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>03/04/2018</td>
                                <td>Corrida - 100m</td>
                                <td>500 Cal</td>
                                <td><a href='?mod=rtreinos&sub=editar_gasto_calorico'><button type="button" class="btn btn-primary">Editar</button> </a> </td> 
                                <td><button type="button" class="btn btn-danger" href="#" data-toggle="modal" data-target="#myModal2">Excluir</button></td> 
                            </tr>
                            <tr>
                                <td>05/04/2018</td>
                                <td>Corrida - 100m</td>
                                <td>500 Cal</td>
                                <td><a href='?mod=rtreinos&sub=editar_gasto_calorico'><button type="button" class="btn btn-primary">Editar</button> </a> </td> 
                                <td><button type="button" class="btn btn-danger" href="#" data-toggle="modal" data-target="#myModal2">Excluir</button></td>
                            </tr>
                            <tr>
                                <td>07/04/2018</td>
                                <td>Corrida - 100m</td>
                                <td>700 Cal</td>
                                <td><a href='?mod=rtreinos&sub=editar_gasto_calorico'><button type="button" class="btn btn-primary">Editar</button> </a> </td> 
                                <td><button type="button" class="btn btn-danger" href="#" data-toggle="modal" data-target="#myModal2">Excluir</button></td>
                            </tr>
                            <tr>
                                <td>08/04/2018</td>
                                <td>Corrida - 100m</td>
                                <td>750 Cal</td>
                               <td><a href='?mod=rtreinos&sub=editar_gasto_calorico'><button type="button" class="btn btn-primary">Editar</button> </a> </td> 
                                <td><button type="button" class="btn btn-danger" href="#" data-toggle="modal" data-target="#myModal2">Excluir</button></td>
                            </tr>
                            <tr>
                                <td>12/04/2018</td>
                                <td>Corrida - 100m</td>
                                <td>805 Cal</td>
                              <td><a href='?mod=rtreinos&sub=editar_gasto_calorico'><button type="button" class="btn btn-primary">Editar</button> </a> </td> 
                                <td><button type="button" class="btn btn-danger" href="#" data-toggle="modal" data-target="#myModal2">Excluir</button></td>
                            </tr>
                            <tr>
                                <td>18/04/2018</td>
                                <td>Corrida - 100m</td>
                                <td>900 Cal</td>
                               <td><a href='?mod=rtreinos&sub=editar_gasto_calorico'><button type="button" class="btn btn-primary">Editar</button> </a> </td> 
                                <td><button type="button" class="btn btn-danger" href="#" data-toggle="modal" data-target="#myModal2">Excluir</button></td>
                            </tr>
                            <tr>
                                <td>Total</td>
                                <td></td>
                                <td>4155 Cal</td>
                                <td></td>
                                <td></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </section>  
        <footer class="footer bg-dark text-white">
            <div class="container center ">
                <div class="row justify-content-center ">
                    <span>© 2018 - Todos os Direitos Reservados - Mais Saúde São João</span>
                    <br>
                    <span>Desenvolvido por alunos do <a href="https://sbv.ifsp.edu.br/">Instituto Federal de Educação Ciência e Tecnologia de São Paulo - Campus São João da Boa Vista</a></span>
                </div>
            </div>
        </footer>
    </body>
</html>