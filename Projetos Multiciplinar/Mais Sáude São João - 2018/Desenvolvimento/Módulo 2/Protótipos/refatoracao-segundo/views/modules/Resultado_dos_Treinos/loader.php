...
$sub_router_content     = file_get_contents(VIEWS_PATH.'modules/Resultado_dos_Treinos/sub_router.json');
$sub_router_object      = json_decode($sub_router_content);

foreach($sub_router_object as $link => $dir) {
    if($load_subdir === $link) include(VIEWS_PATH.'modules/Resultado_dos_Treinos/'.$dir);
}
