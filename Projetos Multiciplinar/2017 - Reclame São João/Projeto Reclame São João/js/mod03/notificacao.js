	...
	xhr.get('php/mod03/notificacao.php?acao=verificar', function(total){
	total_not.innerHTML = total;
	});
	}, 1000);

	window.setInterval(function(){
	xhr.get('php/mod03/notificacao.php?acao=getnots', function(nots){
		res.innerHTML = nots;
	});
	}, 1000);
	...
	if(elemento.classList.contains('vis')){
		xhr.get('php/mod03/notificacao.php?acao=vis&idnot='+elemento.id, function(res){
		alert(res);
		});
	}
	});
