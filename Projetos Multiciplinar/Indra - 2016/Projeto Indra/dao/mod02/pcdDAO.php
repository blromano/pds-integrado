...
class PcdDao extends ConectarBD implements ModeloDAO {
	...
	public function salvarAlteracao($entidade) {

        try {
            $pdo = parent::CriarConexao();

      $sql = "update pcds set pcd_status_funcionamento = :status where pcd_id = :id ;";

            $prepare = $pdo->prepare($sql);

            $prepare->bindValue(":id", $entidade->getId(), PDO::PARAM_INT);
            $prepare->bindValue(":status", $entidade->getPCD_STATUS_FUNCIONAMENTO(), PDO::PARAM_STR);
