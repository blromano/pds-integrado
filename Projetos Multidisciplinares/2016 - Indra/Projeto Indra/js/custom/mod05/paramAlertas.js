	...
	(function($) {
		FormValidation.Validator.verificarValor= {
			validate: function(validator, $field, options) {
				...
				var valorMax = $field.parents().parents().children().eq(0).children().eq(1).val();
	...
	(function($) {
		FormValidation.Validator.min= {
			validate: function(validator, $field, options) {
				...
				var valorMax = $field.parents().parents().children().eq(2).children().eq(1).val();
...
function listarParam() {
	...
	$.ajax({//Ajax para pegar as informaçõs dos parametros dos sensores da PCD requirida pelo usuário.
		url: '../../class/mod05/paramAlertas/listarParam.php',
		dataType: 'json',
		method: 'POST',
		data: {
			id: idPCD
	...
	$('#simEditar').click(function (){
		...
		if(x){
			...
			if ($('#formEditar').data('formValidation').isValid() || $('#formEditar').data('formValidation').isValid() == null) {
				$.ajax({
					method: 'POST',
					url: '../../class/mod05/paramAlertas/editarParam.php',
					data: para,
					dataType: 'json',
					success: function(data){
						$('.formParam').data('formValidation').resetForm();
	...
	$('#cadastrarSim').click(function (){
		...
		if(x){
			...
			if ($('#formCriar').data('formValidation').isValid()) {
				$.ajax({
					method: 'POST',
					url: '../../class/mod05/paramAlertas/createParam.php',
					data: param,
					success: function(data) {
...
$('#corpoTabela').on( 'click', '.tableInterna a.btn-deletar', function () {
	...
	$('#simDeletar').click(function (){
		if(x){
			$.ajax({
				type: 'POST',
				url: '../../class/mod05/paramAlertas/deletarParam.php',
				data: {
					id: id
				},
				success: function(data){
...
function listarPCD() {
	$.ajax({
		url: '../../class/mod05/paramAlertas/listarPCD.php',
		dataType: 'json',
		success: function(data){
			if(data.length > 0){
				t.clear().draw();
