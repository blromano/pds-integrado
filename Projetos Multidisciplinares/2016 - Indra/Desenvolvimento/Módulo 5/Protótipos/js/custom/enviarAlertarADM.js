$( document ).ready(function() {
    ...
    function listarAlertas() {
        $.ajax({
            url: "class/alertas/ADM/listarAlertas.php",
            dataType: "json",
                ...
                if (qntResultados> 0 ) {
                    for (var i = 0; i < qntResultados; i++ ) {
                        t.row.add([
                            ...
                            btnTrue = (data[i].status == "Aprovado") ? "<button id=\"" + data[i].id + "\" data-toggle=\"modal\" data-target=\"#modalValidacao\" class=\"btn btn-primary btn-table btn-valid\">Em validação</button>" : "<button id=\"" + data[i].id + "\" data-toggle=\"modal\" data-target=\"#modalValidarTrue\" class=\"btn btn-primary btn-table btn-true\">Aprovar</button>",
                            btnFalse = (data[i].status == "Reprovado") ? "<button id=\"" + data[i].id + "\" data-toggle=\"modal\" data-target=\"#modalValidacao\" class=\"btn btn-primary btn-table btn-valid\">Em validação</button>" : "<button id=\"" + data[i].id + "\" data-toggle=\"modal\" data-target=\"#modalValidarFalse\" class=\"btn btn-primary btn-table btn-false\">Reprovar</button>"
                        ]).draw();
                    }
                }
            }
    ...
    $('#tableUser tbody').on('click', 'button.btn-true', function() {
        ...
        $('#validTrue').on('click', function() {
            $.ajax({
                url: "class/alertas/ADM/validarAlerta.php",
                type: "POST",
                data: {
                    id: id,
                    valid: "sim"
    ...
    $('#tableUser tbody').on('click', 'button.btn-valid', function() {
        $('#modalValidacao').trigger('next.m.1');
        var id = this.id;
        $('#valid').click(function() {
            $.ajax({
                ...
                success: function( data ) {
                    if(data == true) {
                        $('#modalValidacao').trigger('next.m.2');
                        listarAlertas();
                    }else $('#modalValidacao').trigger('next.m.3');
                }
            });
        });
    });
