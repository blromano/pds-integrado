...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	if(!empty($_POST['nome'])){
		if ($tiposReclamacaoDAO->pesquisar_igual($_POST['nome']) > 0) {
			echo "<script>alert('Categoria ja existente!')</script><script>window.location='../../../index.php?pagina=9';</script>";
		}else{
			$tiposReclamacao->setCategoria($_POST['nome']);
			$tiposReclamacaoDAO->inserir($tiposReclamacao,$vincular);
			echo "<script>alert('Categoria inserida com sucesso!')</script><script>window.location='../../../php/mod05/index.php?pagina=9';</script>";
		}
	}else{
