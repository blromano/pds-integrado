...
use App\Model\PacienteModel;
    ...
    class PacienteDAO extends DAO{
        ...
        // APAGAR ESSA MERDA AQUI - By: Fernanda
        ...
        public function listar(){
            try {
                $pacientes = array();
                $sql = "SELECT pac.PAC_ID, pac.PAC_PRONTUARIO, pac.FK_USUARIOS_USU_ID, pac.FK_PLANOS_PLA_ID, usu.USU_CPF, usu.USU_EMAIL, usu.USU_NOME, pag.PAG_DATAPAGAMENTO, pag.PAG_DATAVENCIMENTO FROM pacientes as pac 
                            INNER JOIN usuarios as usu on (usu.USU_ID = pac.FK_USUARIOS_USU_ID)
                            INNER JOIN pagamentos as pag on (pag.FK_PACIENTES_PAC_ID  = pac.PAC_ID)
                            ORDER BY PAC_ID ASC";
                ...
                foreach ($results as $row) {
                    ...
                    $paciente->__set('USU_CPF', $row['USU_CPF']);
                    $paciente->__set('USU_EMAIL', $row['USU_EMAIL']);
                    $paciente->__set('USU_NOME', $row['USU_NOME']);
                    $paciente->__set('PAG_DATAPAGAMENTO', $row['PAG_DATAPAGAMENTO']);
                    $paciente->__set('PAG_DATAVENCIMENTO', $row['PAG_DATAVENCIMENTO']);
